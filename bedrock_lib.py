...
def get_index(): #creates and returns an in-memory vector store to be used in the application
    ...
    #store.embedding_function.embed_query("")
    wrapper = VectorStoreIndexWrapper(vectorstore=store)
    #wrapper.query()

    index_from_loader = index_creator.from_loaders([loader]) #create an vector store index from the loaded PDF
    #index_creator.from_documents()
    
    return index_from_loader #return the index to be cached by the client app



def get_index1(): #creates and returns an in-memory vector store to be used in the application
    load_dotenv() 
    embeddings = BedrockEmbeddings(
        credentials_profile_name=os.environ.get("BWB_PROFILE_NAME"), #sets the profile name to use for AWS credentials (if not the default)
        region_name=os.environ.get("BWB_REGION_NAME"), #sets the region name (if not the default)
